...
router.post('/', checkAdmin, async (req, res) => {
    ...
    if (!tord_id || !worked_message || !status) {
            return res.status(400).json({ error: 'tord_id, worked_message, and status are required' });
        }
...
router.put('/:id', checkAdmin, async (req, res) => {
    ...
    if (!worked_message || !status) {
            return res.status(400).json({ error: 'worked_message and status are required' });
        }
